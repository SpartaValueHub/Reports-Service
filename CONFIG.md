...
- Eureka instance-id: `서비스명:실제포트` (예: `reports-service:51234`) — 기동 로그와 Eureka 대시보드에서 확인
...
jdbc:mysql://<host>:<port>/reports_db?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Seoul&characterEncoding=UTF-8
...
# 2. .env 설정 후 Reports Service 실행

cd ../reports-service && ./gradlew bootRun
...
- IDE에서 실행할 때는 **Run Configuration** `ReportsServiceApplication` 을 사용하거나, `ReportsServiceApplication.java` 에서 main 실행 시 Working directory 가 프로젝트 루트인지 확인하세요.

- `application-local.yml` 이 `optional:file:.env[.properties]` 를 import 하므로 IDE main 실행 시에도 `.env` 가 로드됩니다.



Eureka Dashboard (`http://localhost:8761`) 에서 `REPORTS-SERVICE` 등록을 확인합니다.
...
| `SPRING_DATASOURCE_URL` | MySQL JDBC URL (`reports_db` 스키마) |
